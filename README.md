...
Xiangzi MD 1.0 是基于 Tauri 2、Rust、React 和 Milkdown 的本地优先 Markdown 编辑器。Electron 旧版保存在仓库的 `electron-legacy-v0.1` 分支；`main` 从 1.0.0 起维护 Tauri 版本。

编辑器、工作区、搜索、设置、菜单、单实例、退出保护以及 HTML/PDF/PNG/JPG 导出已经迁移。PDF 当前采用分页位图，完整文档可导出；可选择文字和可点击链接仍在原生导出适配计划中。详细完成度见 [功能对照表](docs/FEATURE_PARITY.md)。
