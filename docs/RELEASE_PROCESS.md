...
4. `Build`：macOS Universal、macOS arm64、macOS x86_64 和 Windows x64 并行构建、签名，并生成平台校验和、SBOM 与 provenance attestation。
5. `Publish unified GitHub Release`：等待四个构建完成，验证完整产物集，将附件文件名中的空格统一规范化为点号；`latest.json` 继续指向 macOS Universal 归档作为自动更新包，原生 arm64/x86_64 仅作为 DMG 手动下载包。Release 只发布一份合并后的 `SHA256SUMS.txt`；SBOM 保留在构建 provenance 中，不作为 Release 附件。
...
- 3 个 DMG、1 个 EXE、1 个 Universal `.app.tar.gz` 和两个 `.sig` 齐全
- 单份 `SHA256SUMS.txt` 齐全；SBOM 可从对应构建 provenance 查询
