...
4. `Build`：macOS Universal、macOS arm64、macOS x86_64 和 Windows x64 并行构建、签名，生成校验和、SBOM、attestation，并上传 7 天保留的临时 Actions Artifact。
5. `Publish unified GitHub Release`：等待四个构建完成，验证完整产物集，将附件文件名中的空格统一规范化为点号；`latest.json` 继续指向 macOS Universal 归档作为自动更新包，原生 arm64/x86_64 包作为额外 Release 附件供手动下载。
