...
适用版本：Xiangzi MD 1.6.7

> 本文档按 1.6.7 当前源码、菜单实现、设置模型与组件实现重新整理。首次测试时曾遇到开发态窗口未出现，随后复测 `npm run tauri:dev` 已可正常打开；因此截图应继续以可打开的当前窗口补拍，不能使用旧版截图。
...
图 10-2：编辑器设置页面。当前版本可直接看到标题自动编号、自动保存、顶部工具栏、表格列宽布局、编辑时自动调整表格和选中文本快捷工具栏等开关或选项。
...
当前 1.6.7 设置中未发现独立“字体、字号、行高”设置项；可通过“自定义主题 CSS”实现。
