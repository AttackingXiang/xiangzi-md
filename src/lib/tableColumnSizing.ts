...
/**
 * Mainstream `fitGridWidth` sizing: satisfy min-content tracks first, grow toward
 * max-content in proportion to each track's growth potential, then share surplus.
 */
export function fitColumnsToContainer(
  ...
  if (available < minTotal) {
    // The grid cannot satisfy min-content. Keep every column usable and divide
    // the remaining pixels according to its min-content pressure.
...
/** Mainstream `fitCellContents` sizing with practical spreadsheet-style caps. */
