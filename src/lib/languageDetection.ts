...
export function autoDetectLanguage(code: string): string | null {
  ...
  if (
    /^(SELECT|INSERT\s+INTO|UPDATE\s+\w|DELETE\s+FROM|CREATE\s+(TABLE|DATABASE|INDEX)|DROP\s+(TABLE|DATABASE))\b/i.test(
      first,
    )
  )
    return 'SQL'
  ...
  const scores: [string, number][] = [
    ...
    [
      'CSS',
      h(
        /\b(color|background(-color)?|font-(size|family|weight)|margin|padding|display|flex|grid|border)\s*:/,
      ) *
        8 +
